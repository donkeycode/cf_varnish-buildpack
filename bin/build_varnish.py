...
virtualenv_check ()
{
  if [ -z "$VIRTUAL_ENV" ]; then
    echo "Detected VirtualEnv: Please visit https://packagecloud.io/varnishcache/varnish41/install#virtualenv"
  fi
}
...
abort_already_configured ()
{
  if [ -e "$HOME/.pip/pip.conf" ]; then
    if grep -q "varnishcache/varnish41" "$HOME/.pip/pip.conf"; then
...
pip_extra_url ()
{
  printf "[global]\nextra-index-url=https://packagecloud.io/varnishcache/varnish41/pypi/simple\n"
}
